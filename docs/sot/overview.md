...
## 项目是什么
`lifegame-plus` 是一个纯前端 Web 版“生命游戏”（Conway's Game of Life）MVP：固定 100×100 网格、默认环绕边界（toroidal）、初始状态为 glider；支持 `Play/Pause`、播放速率选择，并提供右侧可视化规则面板（规则集下拉 + 原子规则 A→B 卡片）在暂停时切换演变规则（`B.../S...`）。
