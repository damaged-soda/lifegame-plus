...
- 自测要点：
  - 页面加载后会在画面最底行随机出现 A/B/C 三个单点（颜色不同；默认允许同格叠加）；点击 `播放` 开始演进，再次点击暂停（左右环绕、上下不环绕）
  - 通过 `速率` 下拉选择 x0.5/x1/x2/x5/x10/x20/x50/x100，播放中切换后立即生效
  - 播放中规则面板禁用；暂停后可通过规则卡片启用开关切换规则，切换后对 A/B/C 同时生效，并从当前棋盘继续演进（不重置）
  - 暂停后可在“元规则”中开启/关闭：
    - `单格互斥`：开启瞬间会将当前盘面中的同格叠加随机规整为单类型；开启后演进过程中不再出现同格多类型
    - `到顶结束`：任一类型到达顶行时自动结束并禁用播放；通过 `重置/重试/修改 seed` 重新初始化后可重新开始
  - 方向规则新增可选出生规则：`支撑生长（需要下/左下/右下）`（默认关闭；仅当同类型的下/左下/右下均为活时，该格才会通过此规则出生）
